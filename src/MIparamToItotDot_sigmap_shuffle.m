...
for ii=1:numel(rise_params)
    ...
    for rr=1:size(rise,1)
        ...
        [Ip2sig_freq, ip2sig_sup]=histcounts(Ip2sig_dist_shuffle(:), 0:.01:1);
        ...
        if debug
            ...
            figure(2); gcf;
            subplot(4,3,jj);
            area(ip2sig_sup, pIp2sig, 'facecolor', 'k', 'facealpha', 0.1);
            hold on;
            % one-tailed test visual with alpha
            cpI=cumsum(pIp2sig);
            idx=find(cpI>1-alpha);
            
            [val, p_idx]=min(sqrt((ip2sig_sup-mean(Ip2sig_dist(:))).^2));
            if val < 0.01
                pval=1-cpI(p_idx);
            else
            ...
            plot([ip2sig_sup(idx(1)),ip2sig_sup(idx(1))], [0,1], 'k:', 'linewidth', 2);
            ...
            fprintf('Rise (%s to %s), $I(\\dot{I};%s)$,%.3f,%.3f, %.4f\n',...
                    datestr(dateData(rise(rr,1))), datestr(dateData(rise(rr,2))), ...
                    param_label,mean(Ip2sig_dist(:)), ip2sig_sup(idx(1)), pval);    
...
for ii=1:numel(fall_params)
    ...
    for ff=1:size(fall,1)
        ...
        if debug
            ...
            fprintf('Fall (%s to %s), $I(\\dot{I};%s)$,%.3f,%.3f, %.4f\n',...
                    datestr(dateData(rise(rr,1))), datestr(dateData(rise(rr,2))), ...
                    param_label,mean(Ip2sig_dist(:)), ip2sig_sup(idx(1)), pval); 
%             title(sprintf('p=%.5f', pval)); % comment after checking
            set(gca, 'fontsize', 16, 'xlim', [0 1]);
