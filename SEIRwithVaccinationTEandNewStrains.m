...
omega = 0.01;
tau = 100;
%kappa = 0.001*ones(1,numel(tspan));
kappa = square(0.01*tspan);
noise = 10;
...
while iter <= runs
    ...
    [S1,E1,I1,Sm1,Em1,Im1,R1,D1,V1,phi_S1,phi_Sm1,lambda_m1,lambda1]=seir_sim_wrapper_kappa(S0,E0,I0,Sm0, ...
        Em0,Im0,eta_s,eta_i,sigma,beta,epsilon,gamma,mu,alpha,a,b,tau,omega,kappa,noise,tspan);
...
yyaxis right
h3 = plot(meanCoupling,'-b');
legend([h1,h2,h3],'$TE_{\dot{Sm}\rightarrow \dot{S}}$','Null $TE_{\dot{Sm} \rightarrow \dot{S}}$','$\phi_{Sm} S_m$','interpreter','latex');
ylabel('TE (bits)');
xlabel('Window #')
set(gca, 'fontsize', 20);
ylim([-inf inf]);
...
coupling =(1-omega)*sigma.*E(1:end-1,:);
meanCoupling = mean(coupling');
stdCoupling = std(coupling');
